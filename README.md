...
앞단에 대한 관심으로 리액트 사용해야겠다는 마음이 생겼고, 이를 팀원분과 팀을 꾸리게 되어 더욱 책임감을 가지고 진행했지만, 
어려움이 두가지가 있었다.
프로젝트를 시작하면서 리액트를 학습을 시작한 어려움과
rest api, 파일 업로드, 세션처리, aws 서버 배포, 코드를 다시 리팩토 하는 등
모든 영역이 새롭게 접하는 부분이라는 것이었다.
이는 문제 이슈 뿐만 아니라, 그 기술을 왜 도입해야하는지 어느 부분에서 쓸모가 있는지 같은 정말 근본적인 부분도 몰랐던 것들을
알아내야 한다는 것이었다. 흐름과 원리가 중요한 나에게는 어려움이 있었다.
지금은 당연한 rest로 html 페이지가 아닌 json을 주고 받는다는 개념은 json을 안쓰는 java로 html페이지만 변환하던 내게는 어려웠따.

그렇게 학원에서는 팀원분과 정말 많은 대화로 커뮤니케이션을 하며, 프로젝트를 짜나갔고, 집에서는 기술들을 학습하고, 
개발을 하며 서로 만족하는 결과를 얻었다.
리드해주신 선생님 평가 1등을 얻게 되었다. 
더욱이 처음에 설정한 배포라는 목표까지 결과 발표를 할 수 있었다.

다만, 2개월이라는 짧은 시간동안 쇼핑몰에 관한 모든 것을 커버하느라 넓은 영역을 얕게 구현한 것 같다.
도입하고 싶은 기술이 있어도, 팀원분과 기술적 합의가 있었어야 했다. 이번에는 jwt가 그랬다.

다음 프로젝트는 익히는 기술을 깊이 학습하고, 쓰임새대로 제대로 적용하고 싶다.
